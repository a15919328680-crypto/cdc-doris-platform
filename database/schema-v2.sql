...
CREATE TABLE IF NOT EXISTS `database_connection` (
    ...
    INDEX `idx_type` (`type`),
    INDEX `idx_role` (`role`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='数据库连接配置表（MySQL/Doris 通用）';
...
CREATE TABLE IF NOT EXISTS `sync_task` (
    ...
    `source_table` VARCHAR(100) NOT NULL COMMENT '源表名，支持正则表达式（如 user_*）',
    ...
    `sync_mode` VARCHAR(20) DEFAULT 'FULL' COMMENT '同步模式：FULL(全量), INCREMENTAL(增量), CDC(全量 + 增量)',
...
-- ('本地 MySQL', 'MYSQL', 'localhost', 3306, 'root', 'password', 'BOTH', '本地 MySQL 数据库'),
-- ('Doris 集群', 'DORIS', '192.168.1.100', 9030, 'root', '', 'TARGET', 'Doris 前端节点');
